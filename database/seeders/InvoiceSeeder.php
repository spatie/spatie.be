...
class InvoiceSeeder extends Seeder
{
    public function run(): void
    {
        ...
        User::each(function (User $user) use ($products) {
            foreach ($products->take(3) as $index => $product) {
                ...
                $existingPurchase = $user->purchases()->where('purchasable_id', $purchasable->id)->first();

                if ($existingPurchase) {
                    $existingPurchase->update(['receipt_id' => $receipt->id]);
                } else {
                    Purchase::create([
                        'user_id' => $user->id,

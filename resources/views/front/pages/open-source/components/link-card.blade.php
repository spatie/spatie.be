...
<div
    {{ $attributes->merge([
        'class' => 'relative group rounded-[20px] p-7 md:p-10 text-[14px] md:text-[16px] leading-normal ' . ($light ? 'text-oss-royal-blue' : 'link-card text-oss-gray shadow-oss-card'),
    ])->except(['href', 'target']) }}
>
    @if ($light)
    ...
    {{ $slot }}
    @if ($href)
    <a class="flex items-center gap-x-2 mt-9" href="{{ $href }}" @if($target) target="{{ $target }}" @endif>

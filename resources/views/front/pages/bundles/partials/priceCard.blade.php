@php
    $priceWithoutDiscount = $bundle->getPriceWithoutDiscountForCurrentRequest();
    $price = $bundle->getPriceForCurrentRequest()
@endphp
<div
    ...
    <div class="flex-grow markup markup-lists markup-lists-compact text-xs">
        @if ($bundle->originalPurchasable)
            {!! $bundle->originalPurchasable->formattedDescription !!}
        @else
            {!! $bundle->formattedDescription !!}
        @endif
    </div>

    @if ($bundle->hasActiveDiscount())
        ...
        <div class="-mx-6 px-2 py-3 bg-green-lightest mt-4 text-black text-sm text-center">
            @if ($bundle->discount_name)
                <span>{{ $bundle->discount_name }} <span class="char-separator">•</span> </span>
            @endif
            Now <span class="font-semibold">{{ $bundle->displayableDiscountPercentage() }}%</span> off
            @if(optional(current_user())->enjoysExtraDiscountOnNextPurchase())
                for you
            @endif

            @if(optional($bundle->currentDiscountPercentageExpiresAt())->isFuture())
                <div
                    class="mt-1 text-green-dark text-xs"
                    style="font-variant-numeric:tabular-nums">
                    <x-countdown :expires="$bundle->currentDiscountPercentageExpiresAt()">
                        <span><span
                                x-text="timer.days"
    ...
    <div class="flex-0 mt-6 flex justify-center">
        <div class="w-full flex justify-center">
            @auth
                @if (isset($payLink))
                    ...
                    <script type="text/javascript">
                        ...
                        let options = {
                            ...
                            method: 'inline',
                            quantity: 1,
                            product: {{ $bundle->paddle_product_id }},
                            disableLogout: true,
                ...
                @else
                    <a href="{{ route('products.buy', [$product, $bundle]) }}">
                        <x-button :large="true">
                ...
                @endif
            @else
                <a href="{{ route('login') }}?next={{ route('products.buy', [$product, $bundle]) }}">
                    <x-button :large="true">
                        <span class="font-normal">Buy for&nbsp;</span>
                        <span>{{ $price->formattedPrice() }}</span>
                    </x-button>

    ...
    <ol class="text-xs grid gap-2 links-blue markup-list-compact">
        ...
        <li class="{{ isset($lesson) && $lesson->id === $lessonInChapter->id ? " font-sans-bold" : "" }}">
            <a class="block" href="{{ route('courses.show', [$series, $lessonInChapter]) }}">
                ...
                @if($lessonInChapter->display === \App\Models\Enums\LessonDisplayEnum::SPONSORS && !
                $lessonInChapter->canBeSeenByCurrentUser())
                ...
                @if($lessonInChapter->display === \App\Models\Enums\LessonDisplayEnum::LICENSE && !
                $lessonInChapter->canBeSeenByCurrentUser() )
                ...
                @if($lessonInChapter->display === \App\Models\Enums\LessonDisplayEnum::AUTH && !
                $lessonInChapter->canBeSeenByCurrentUser() )
                ...
                <span title="Completed" style="left: calc(-1.5em - 1.5rem); top: 0.075rem"
                    class="absolute w-4 h-4 inline-flex items-center justify-center bg-green rounded-full">
